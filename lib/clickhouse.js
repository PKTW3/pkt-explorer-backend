...
const types = (Object.freeze((() => {
  ...
  _.Enum = (enums /*:{[string]:number}*/) => {
    return {
      _flow: Object.keys(enums).map(x => `'${x}'`).join('|'), ch: `Enum(${Object.keys(enums).map((t) => `'${t}' = ${String(enums[t])}`).join(', ')
        })`, writable: true
    };
  };
...
module.exports.IF_NOT_EXISTS = IF_NOT_EXISTS;
const TEMPORARY = ({
  string: 'TEMPORARY',
  type: 'Option'
} /*:Option_t*/);
...
const table = /*::<X>*/(obj /*:TableDesc_t*/) /*:Table_t<X>*/ => {
...
module.exports.database = () /*:Database_t*/ => {
  const tables = {};
  const tempTables = {};
  return Object.freeze({
    tables: () => tables,
...
const insert = /*::<X:Object>*/(
  ...
  const startTime = +new Date();
  nThen((w) => {
    // $FlowFixMe: 2 argument zlib.gzip is a thing
    Zlib.gzip(objs.join('\n') + '\n', w((err, ret) => {
      if (err) {
        w.abort();
        return void then(err);
...
const create = (
  ...
  const out = {
    _: ch,
    opts: conf,
    log: Log.create('db', conf.logLevel || 'info'),
    ...
    withSession2: (id /*:?string*/) => {//FIXES "DB::Exception: Session is locked by a concurrent" SESSION ID CANNOT BE REUSED IN CLICKHOUSE WITH CONCURRENT CONNECTIONS
      return create(Object.assign({}, conf, { sessionId: randId() }), ch);//forces a new connection with a random id
    },
    withDb: (db /*:string*/) => {
